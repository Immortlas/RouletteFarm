...
local newFrame = imgui.OnFrame(
    ...
    function(player)
		...
		imgui.BeginChild("##rullet", imgui.ImVec2(230, 84), true, imgui.WindowFlags.NoScrollbar)
			if imgui.Checkbox(u8'Îáû÷íûé', chest.common.open) then
				mainIni.chest_open.common = chest.common.open[0]
				inicfg.save(mainIni)
			end
			imgui.SameLine()
			imgui.TextQuestion("( ? )", u8"ßùèê, êîòîðûé âûäàåòñÿ çà ïðîõîæäåíèå íà÷àëüíûõ êâåñòîâ")
			if chest.common.sec > 0 then
			imgui.SameLine(160)
			imgui.Text(string.format("%dm : %ds ", math.floor(chest.common.sec/60), chest.common.sec%60))
			end

			if imgui.Checkbox(u8'Äîíàòíûé', chest.donate.open) then
				mainIni.chest_open.donate = chest.donate.open[0]
				inicfg.save(mainIni)
			end
			imgui.SameLine()
			imgui.TextQuestion("( ? )", u8"ßùèê, êîòîðûé ïîêóïàåòñÿ â äîíàò-ìåíþ çà 6.000 AZ ìîíåò")
			if chest.donate.sec > 0 then
			imgui.SameLine(160)
			imgui.Text(string.format("%dm : %ds ", math.floor(chest.donate.sec/60), chest.donate.sec%60))
			end

			if imgui.Checkbox(u8'Ïëàòèíîâûé', chest.platinum.open) then
				mainIni.chest_open.platinum = chest.platinum.open[0]
				inicfg.save(mainIni)
			end
			imgui.SameLine()
			imgui.TextQuestion("( ? )", u8"ßùèê, êîòîðûé âûäàåòñÿ ïîñëå òîãî, êàê âû îòûãðàåòå 360 ÷àñîâ")
		...
		imgui.BeginChild("##settings", imgui.ImVec2(230, settings_y), true, imgui.WindowFlags.NoScrollbar)
			if imgui.Checkbox(u8'Ñîîáùåíèÿ â ÷àò', settings.chat) then
				mainIni.settings.chat = settings.chat[0]
				inicfg.save(mainIni)
			end
			imgui.SameLine()
			imgui.TextQuestion("( ? )", u8"Âûâîä ñîîáùåíè¤, ñâÿçàííûé ñ ðàáîîòé ñêðèïòîì, â ÷àò ñàìïà")

			if imgui.Checkbox(u8'Âðåìÿ äî îòêðûòèÿ', settings.time_on_screen) then
				mainIni.settings.time_on_screen = settings.time_on_screen[0]
				inicfg.save(mainIni)
			end
			imgui.SameLine()
			imgui.TextQuestion("( ? )", u8"Âûâîä íà ýêðàí âðåìåíè äî îòêðûòèÿ ðóëåòêè")

			if settings.time_on_screen[0] and settings.enabled then
				settings_y = 85
				main_y = 100
				if imgui.Button(u8"Èçìåíèòü ìåñòîïîëîæåíèå", imgui.ImVec2(200, 20)) then
					chat_msg(string.format("Íàæìèòå êëàâèøó Space, ÷òîáû ñîõðàíèòü ïîçèöèþ"), 0xFFE4B5)
		...
		local close_button_text = settings.enabled and u8"Äåàêòèâèðîâàòü" or u8"Àêòèâèðîâàòü"
...
function start()
	settings.enabled = not settings.enabled
		if settings.enabled then
			...
			settings.close_invent = false
			settings.open_invent = false
...
function main()
	...
	while true do
		...
		if settings.time_on_screen[0] then
			sampTextdrawSetString(222, string.format("%s-%d:%d~n~%s-%d:%d~n~%s-%d:%d",
			chest.common.name, math.floor(chest.common.sec/60), chest.common.sec%60,
			chest.platinum.name, math.floor(chest.platinum.sec/60), chest.platinum.sec%60,
			chest.donate.name, math.floor(chest.donate.sec/60), chest.donate.sec%60
			))
		end
		...
		if settings.enabled and sampIsLocalPlayerSpawned() and not settings.change_time_position then
			...
			for _, val in pairs(chest) do
			--wait(300)
				--chat_msg(string.format("%s %s %s %s %s", tostring(val.check_time), tostring(val.open[0]), tostring(settings.open_chest), tostring(settings.enabled), val.name), 0xFFE4B5)
				if val.check_time and val.open[0] and not settings.open_chest and settings.enabled then
					--chat_msg("Lol Kek", 0xFFE4B5)
...
if settings.enabled and sampIsLocalPlayerSpawned() then
	...
	if data.modelId == 1353 then  -- Ïëàòèíîâûé ñóíäóê
	...
	if data.modelId == 1240 and tostring(data.zoom) == "0.89999902248383" then -- Ñóíäóê âàëåíòèíà
	...
	if data.modelId == 19613 then -- Ñóíäóê çà 6.000 AZ
	...
	if data.modelId == 19918 then -- Îáû÷íûé ñóíäóê
